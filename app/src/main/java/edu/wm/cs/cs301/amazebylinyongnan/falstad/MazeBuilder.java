...
import edu.wm.cs.cs301.amazebylinyongnan.falstad.Constants;
...
public class MazeBuilder implements Runnable {
	...
	public void run() {
		...
		try {
            ...
            //--------------- android app ------------------------
			//store the maze if the skill level is 0,1,2,3 (which means width is 4, 12, 15, 20)
            if(width < 21){
                order.storeMaze(width, height, rooms, expectedPartiters, root, cells, dists.getDists(), startx, starty);
            }

            order.notifyController();
            //----------------------------------------------------
