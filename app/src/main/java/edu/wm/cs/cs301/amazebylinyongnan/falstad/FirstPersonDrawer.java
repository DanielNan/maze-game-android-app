...
import edu.wm.cs.cs301.amazebylinyongnan.falstad.BSPBranch;
import edu.wm.cs.cs301.amazebylinyongnan.falstad.BSPLeaf;
import edu.wm.cs.cs301.amazebylinyongnan.falstad.BSPNode;
import edu.wm.cs.cs301.amazebylinyongnan.falstad.CardinalDirection;
import edu.wm.cs.cs301.amazebylinyongnan.falstad.Cells;
import edu.wm.cs.cs301.amazebylinyongnan.falstad.MazeBuilder;
import edu.wm.cs.cs301.amazebylinyongnan.falstad.Seg;
import edu.wm.cs.cs301.amazebylinyongnan.falstad.Wall;
...
public class FirstPersonDrawer extends DefaultViewer {
	...
	public void redraw(MazePanel panel, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		//by calling this method in MazePanel following code is executed.
		//---------------------panel.setSimpleGraphics();----------------
		...
		//calling this method in MazePanel and set the color by string.
	...
	private void drawBackground(MazePanel panel) {
		//by calling this method we set the colors and fillRect by given dimension.
	...
	private void drawSegment(Seg seg) {
		...
		int x1i = x1;
		int xd = x2-x1;
		panel.gcSetColorWithColor(seg.getPanel().getSegColor());
		//panel.getGraphics().setColor(seg.getPanel().segGetColor());
